...
export class AuthService {
  private apiUrl = 'http://localhost:5169/api/auth'; // Ajusta según tu backend
  ...
  logout(): void {
    ...
    this.http.post<AuthModel>(`${this.apiUrl}/logout`, {}, { headers }).subscribe({
      next: () => {
        this.removeToken();
  ...
  deleteUser(userId: number): Observable<AuthModel> {
    const headers = new HttpHeaders({ Authorization: `Bearer ${this.getToken()}` });
    return this.http.delete<AuthModel>(`${this.apiUrl}/delete/${userId}`, { headers });
  }
